function [ x_bounded ] = bounds( params )
    ...
    x_bounded(1) = mintomod_ab(params(1), 0, 0.99); % eta (if eta is large, then ND becomes larger than YD in ss. Thus CD is negative)
    x_bounded(2) = mintomod_ab(params(2), 0.001, 0.999); % gamma
    x_bounded(3) = mintomod_ab(params(3), 0.5, 0.99); % phi
    x_bounded(4) = mintomod_ab(params(4), 0.001, 0.5); % lambda
