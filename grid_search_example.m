...
%% My Extended Example - 3 Params
% firstparam = [1, 2, 3.3, 3.7, 8, 21];  %list of places to search for first parameter
% secondparam = linspace(0,1,3);        %list of places to search for second parameter
% thirdparam = linspace(0,10,2);
% [X1,X2,X3] = ndgrid(firstparam, secondparam, thirdparam);
% fitresult = arrayfun(@(p1,p2,p3) fittingfunction3(p1,p2,p3), X1, X2, X3); %run a fitting on every pair fittingfunction(F(J,K), S(J,K))
% 
% % Find the smallest element
% [F_min, I] = min(fitresult(:))
% [I_row, I_col, I3] = ind2sub(size(fitresult),I)
% 
% p1_opt = X1(I_row, I_col, I3)
% p2_opt = X2(I_row, I_col, I3)
% p3_opt = X3(I_row, I_col, I3)
% f_opt  = fittingfunction3(p1_opt,p2_opt,p3_opt)

% TODO
% can I use this?
% https://www.mathworks.com/matlabcentral/answers/225681-how-to-do-grid-search-to-optimize-sigma-using-matlab

% TODO: modify the function being used, with 8 or 9 inputs, with values as
% I wrote up previously
% TODO: get this working with my distance_fcn()
...
fitresult = arrayfun(@(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10) fittingfunction3(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10), X1,X2,X3,X4,X5,X6,X7,X8,X9,X10); %run a fitting on every pair fittingfunction(F(J,K), S(J,K))
